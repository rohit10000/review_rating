...
class GeneratedClient extends GeneratedClientContext {
    ...
    entitiesRecognitionPii(input, options, callback) {
        return this.sendOperationRequest({
            input,
            options
        }, entitiesRecognitionPiiOperationSpec, callback);
    }
...
function makeTextAnalyticsSuccessResult(id, statistics) {
    return {
        id,
        statistics
    };
...
// Copyright (c) Microsoft Corporation.
function makeDetectLanguageResult(id, detectedLanguages, statistics) {
    return Object.assign(Object.assign({}, makeTextAnalyticsSuccessResult(id, statistics)), { primaryLanguage: primaryLanguage(detectedLanguages) });
}
 ...
 */
function sortByPreviousIdOrder(sortedArray, unsortedArray) {
...
// Copyright (c) Microsoft Corporation.
function makeDetectLanguageResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
        return makeDetectLanguageResult(document.id, document.detectedLanguages, document.statistics);
    })
...
// Copyright (c) Microsoft Corporation.
function makeRecognizeCategorizedEntitiesResult(id, entities, statistics) {
    return Object.assign(Object.assign({}, makeTextAnalyticsSuccessResult(id, statistics)), { entities });
}
...
// Copyright (c) Microsoft Corporation.
function makeRecognizeCategorizedEntitiesResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
        return makeRecognizeCategorizedEntitiesResult(document.id, document.entities, document.statistics);
    })
        .concat(errors.map((error) => {
...
// Copyright (c) Microsoft Corporation.
function makeRecognizePiiEntitiesResult(id, entities, statistics) {
    return Object.assign(Object.assign({}, makeTextAnalyticsSuccessResult(id, statistics)), { entities });
}
...
// Copyright (c) Microsoft Corporation.
function makeRecognizePiiEntitiesResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
...
// Copyright (c) Microsoft Corporation.
function makeAnalyzeSentimentResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
        return makeAnalyzeSentimentResult(document.id, document.sentiment, document.documentScores, document.sentenceSentiments, document.statistics);
    })
        .concat(errors.map((error) => {
...
// Copyright (c) Microsoft Corporation.
function makeExtractKeyPhrasesResult(id, keyPhrases, statistics) {
    return Object.assign(Object.assign({}, makeTextAnalyticsSuccessResult(id, statistics)), { keyPhrases });
}
...
// Copyright (c) Microsoft Corporation.
function makeExtractKeyPhrasesResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
...
// Copyright (c) Microsoft Corporation.
function makeRecognizeLinkedEntitiesResult(id, entities, statistics) {
    return Object.assign(Object.assign({}, makeTextAnalyticsSuccessResult(id, statistics)), { entities });
}
...
// Copyright (c) Microsoft Corporation.
function makeRecognizeLinkedEntitiesResultCollection(input, documents, errors, modelVersion, statistics) {
    const unsortedResult = documents
        .map((document) => {
        return makeRecognizeLinkedEntitiesResult(document.id, document.entities, document.statistics);
    })
        .concat(errors.map((error) => {
...
function createSpan(operationName, operationOptions) {
    ...
    if (span.isRecording()) {
        newSpanOptions = Object.assign(Object.assign({}, tracingOptions.spanOptions), { parent: span, attributes: Object.assign(Object.assign({}, spanOptions.attributes), { "az.namespace": "Microsoft.CognitiveServices" }) });
    }
